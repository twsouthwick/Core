...
namespace Castle.Components.DictionaryAdapter.Xml.Tests
{
		...
		public class XmlAdapterAcceptanceTestCase
	{
		...
		public void Adapter_OnXml_Will_Preserve_References()
		{
			...
			Assert.Same(season.Location, season.Location);
			Assert.Equal(line1, season.Location.Line1);
			...
			Assert.Equal("1234 Tulip", season.Location.Line1);
		}
		...
		public void Adapter_OnXml_CanReadProperties()
		{
			...
			Assert.Equal(minAge, season.MinimumAge);
			Assert.Equal(division, season.Division);
			Assert.Equal(startsOn.Date, season.StartsOn.Date);
			Assert.Equal(endsOn.Date, season.EndsOn.Date);
			Assert.Equal(line1, season.Location.Line1);
			Assert.Equal(city, season.Location.City);
			Assert.Equal(state, season.Location.State);
			Assert.Equal(zipCode, season.Location.ZipCode);
			Assert.Equal(2, season.Teams.Count);

			var team = season.Teams[0];
			var n = team.Name;

			Assert.Equal(team1Name, season.Teams[0].Name);
			Assert.Equal(team1Balance, season.Teams[0].Balance);
			Assert.Null(season.Teams[0].GamesPlayed);
			Assert.Equal(2, season.Teams[0].Players.Count);
			Assert.Equal(team1Player1FirstName, season.Teams[0].Players[0].FirstName);
			Assert.Equal(team1Player1LastName, season.Teams[0].Players[0].LastName);
			Assert.Equal(team1Player2FirstName, season.Teams[0].Players[1].FirstName);
			Assert.Equal(team1Player2LastName, season.Teams[0].Players[1].LastName);
			Assert.Equal(team2Name, season.Teams[1].Name);
			Assert.Equal(team2Balance, season.Teams[1].Balance);
			Assert.Equal(1, season.Teams[1].Players.Count);
			Assert.Equal(team2Player1FirstName, season.Teams[1].Players[0].FirstName);
			Assert.Equal(team2Player1LastName, season.Teams[1].Players[0].LastName);
			Assert.Equal(2, season.TeamsArray.Length);
			Assert.Equal(team1Name, season.TeamsArray[0].Name);
			Assert.Equal(team1Balance, season.TeamsArray[0].Balance);
			Assert.Equal(team2Name, season.TeamsArray[1].Name);
			Assert.Equal(team2Balance, season.TeamsArray[1].Balance);
			Assert.Equal(team1Balance + team2Balance, season.Balance);
			Assert.Equal(team1Name, season.FirstTeamName);
			Assert.Equal("notes", season.Notes); // TODO: Improve
			Assert.Equal(3, season.Tags.Length);
			Assert.Contains(tags[0], season.Tags);
			Assert.Contains(tags[1], season.Tags);
			Assert.Contains(tags[2], season.Tags);
			Assert.NotNull(season.ExtraStuff);
			Assert.Equal(licenseNo, season.ExtraStuff["LicenseNo", "RISE"].InnerText);
		}
		...
		public void Adapter_OnXml_CanWriteProperties()
		{
			...
			Assert.NotNull(season.Location);
			Assert.Null(document.DocumentElement["Address", "Common"]);
			...
			Assert.Equal(state, season.Location.State);
			Assert.Equal(zipCode, season.Location.ZipCode);
			Assert.Equal(3, season.Teams.Count);
			Assert.Equal(team1Name, season.Teams[0].Name);
			Assert.Equal(team1Balance, season.Teams[0].Balance);
			Assert.Equal(team3Name, season.Teams[2].Name);
			Assert.Equal(team3Balance, season.Teams[2].Balance);
			Assert.Equal(notes, season.Notes);

			season.Teams.RemoveAt(1);
			Assert.Equal(2, season.Teams.Count);
			Assert.Equal(team3Name, season.Teams[1].Name);
			Assert.Equal(team3Balance, season.Teams[1].Balance);
		}
		...
		public void Adapter_OnXml_CanCopySubTree()
		{
			...
			Assert.NotSame(season1.Location, season2.Location);
			Assert.Equal(season1.Location.Line1, season2.Location.Line1);
			Assert.Equal(season1.Location.City, season2.Location.City);
			Assert.Equal(season1.Location.State, season2.Location.State);
			Assert.Equal(season1.Location.ZipCode, season2.Location.ZipCode);
			Assert.Equal(season1.Tags.Length, season2.Tags.Length);
			Assert.Equal(season2.Tags[0], season1.Tags[0]);
			Assert.Equal(season2.Tags[1], season1.Tags[1]);
			Assert.Equal(season2.Tags[2], season1.Tags[2]);
			Assert.Equal(season2.Teams.Count, season1.Teams.Count);
			Assert.Equal(season2.Teams[0].Name, season1.Teams[0].Name);
			Assert.Equal(season2.Teams[0].Balance, season1.Teams[0].Balance);
			Assert.Equal(season2.Teams[0].Players.Count, season2.Teams[0].Players.Count);
			Assert.Equal(season2.Teams[0].Players[0].FirstName, season1.Teams[0].Players[0].FirstName);
			Assert.Equal(season2.Teams[0].Players[0].LastName, season1.Teams[0].Players[0].LastName);
			Assert.Equal(season2.Teams[1].Name, season1.Teams[1].Name);
			Assert.Equal(season2.Teams[1].Balance, season1.Teams[1].Balance);
			Assert.Equal(player.FirstName, season1.Teams[0].Players[1].FirstName);
			Assert.Equal(player.LastName, season1.Teams[0].Players[1].LastName);

			season2.Location = null;
			season2.Tags = null;
			Assert.Equal(0, season2.Tags.Length);
		}
		...
		public void Will_Ignore_If_Reassigned_Same_Component()
		{
			...
			Assert.Equal("Rockwall", season.Location.City);
		}
		...
		public void Will_Use_Interface_Name_Without_I_When_Writing()
		{
			...
			Assert.Equal("Team", document.DocumentElement.LocalName);
			Assert.Equal("", document.DocumentElement.NamespaceURI);
		}
		...
		public void Will_Use_TypeName_From_XmlTypeAttribute_When_Writing()
		{
			...
			Assert.Equal("Player", document.DocumentElement.LocalName);
			Assert.Equal("", document.DocumentElement.NamespaceURI);
		}
		...
		public void Will_Use_ElementName_And_Namesace_From_XmlRootAttribute_When_Writing()
		{
			...
			Assert.Equal("Season", document.DocumentElement.LocalName);
			Assert.Equal("RISE", document.DocumentElement.NamespaceURI);
		}

		[Fact, Ignore("Not done")]
		...
		public void Can_Remove_From_Collections()
		{
			...
			Assert.Equal(0, season.Teams.Count);
			...
			Assert.Equal(1, league.Count);
			Assert.Equal(0, league[0].ChildNodes.Count);
		}
		...
		public void Can_Read_Nullable_Attribute()
		{
			...
			var season = CreateXmlAdapter<ISeason>(xml, ref document);
			var team = season.Teams[0];
			Assert.Equal(2, team.GamesPlayed);
		}
		...
		public void Can_Read_Nullable_Element()
		{
			...
			var season = CreateXmlAdapter<ISeason>(xml, ref document);
			var team = season.Teams[0];
			Assert.Equal(15, team.MaxPlayers);
		}
		...
		public void Can_Promote_Namespace_To_Document()
		{
			...
			Assert.Equal("c", document.DocumentElement.GetPrefixOfNamespace("Common"));
		}
		...
		public void Can_Coerce_Xml_Without_Namespace()
		{
			...
			Assert.Equal("200.00", teamNode["AmountDue", ""].InnerText);
		...
		public void Can_Determine_If_Property_Defined_In_Xml()
		{
			...
			XmlDocument document = null;
			var season = CreateXmlAdapter<ISeason>(xml, ref document);
			Assert.True(XmlAdapter.IsPropertyDefined("Location", season));
			Assert.False(XmlAdapter.IsPropertyDefined("Name", season));
		}
		...
		public void Can_Determine_If_Collection_Defined_In_Xml()
		{
			...
			XmlDocument document = null;
			var season = CreateXmlAdapter<ISeason>(xml, ref document);
			Assert.True(XmlAdapter.IsPropertyDefined("Teams", season));
...
//			Assert.IsFalse(XmlAdapter.IsPropertyDefined("Tags", season));
			Assert.True(XmlAdapter.IsPropertyDefined("Tags", season));
		}
		...
		public void Can_Reassign_Lists()
		{
			...
			Assert.Equal(1, season.Teams.Count);
		}
		...
		public void Can_Remove_Properties()
		{
			...
			XmlDocument document = null;
			var season = CreateXmlAdapter<ISeason>(xml, ref document);
			Assert.True(XmlAdapter.IsPropertyDefined("Name", season));
			season.Name = "";
			Assert.False(XmlAdapter.IsPropertyDefined("Name", season));
		}
		...
		public void Can_Read_From_Standard_Xml_Serialization()
		{
			...
			Assert.Equal(2, groupRead.Id);
			Assert.IsInstanceOf<IManager>(groupRead.Owner);
			var managerRead = (IManager)groupRead.Owner;
			Assert.Equal(manager.Name, managerRead.Name);
			Assert.Equal(manager.Level, managerRead.Level);
			var employeesRead = groupRead.Employees;
			Assert.Equal(2, employeesRead.Length);
			Assert.Equal(employee.Name, employeesRead[0].Name);
			Assert.Equal(employee.Job.Title, employeesRead[0].Job.Title);
			Assert.Equal(employee.Job.Salary, employeesRead[0].Job.Salary);
			Assert.Equal(employee.Metadata.Tag, employeesRead[0].Metadata.Tag);
			CollectionAssert.AreEqual(employee.Key, employeesRead[0].Key);
			Assert.Equal(manager.Name, employeesRead[1].Name);
			Assert.IsInstanceOf<IManager>(employeesRead[1]);
			var managerEmplRead = (IManager)employeesRead[1];
			Assert.Equal(manager.Level, managerEmplRead.Level);
			CollectionAssert.AreEqual(group.Tags, groupRead.Tags);
			var extraInfoRead = groupRead.ExtraInfo;
			Assert.Equal(3, extraInfoRead.Length);
			Assert.Equal(group.ExtraInfo[0], extraInfoRead[0]);
			Assert.Equal(group.ExtraInfo[1], extraInfoRead[1]);
			Assert.IsInstanceOf<IManager>(extraInfoRead[2]);
			var managerExtra = (IManager)extraInfoRead[2];
			Assert.Equal(manager.Name, managerExtra.Name);
			Assert.Equal(manager.Level, managerExtra.Level);

			groupRead.Comment = "Hello World";
			Assert.Equal("Hello World", groupRead.Comment);
			var commentRead = document["Comment", "Yum"];
			Assert.Null(commentRead);
		}
		...
		public void Can_Read_Nillables_From_Standard_Xml_Serialization()
		{
			...
			var groupRead = CreateXmlAdapter<IGroup>(null, ref document);
			Assert.Null(groupRead.Owner);
			Assert.Null(groupRead.ExtraInfo);
			Assert.Equal(1, groupRead.Employees.Length);
		}

		[Fact, Ignore("Need to opt in or out of reference tracking for this to work.")]
		public void Can_Write_To_Standard_Xml_Serialization()
		{
			...
			using (var stream = new FileStream("out.xml", FileMode.Open))
			{
				...
				Assert.Equal(manager.Level, managerEmplRead.Level);
				CollectionAssert.AreEqual(group.Tags, groupRead.Tags);
				Assert.Null(groupRead.Comment);
				Assert.Equal(3, groupRead.ExtraInfo.Length);
				Assert.Equal(group.ExtraInfo[0], groupRead.ExtraInfo[0]);
				Assert.Equal(group.ExtraInfo[1], groupRead.ExtraInfo[1]);
		...
		public void Will_Return_Null_if_Must_Exist()
		{
			...
			Assert.Null(employee.Supervisor);
		}
	}
